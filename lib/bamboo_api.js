...
(function() {
  var Dataset, LS, allowed_aggregation, bamboo_url, dbg, ensure_jquery, is_aggregation, jsonify, noop, settings, _add_aggregation, _add_calculation, _create_dataset, _dataset_exists, _delete_dataset, _delete_row, _get_row, _join, _merge, _query, _query_aggregations, _query_calculations, _query_info, _remove_aggregation, _remove_calculation, _reset, _run_query, _summary, _uniqueId, _uniqueIdCount, _update, _update_row,
    __slice = [].slice;
  ...
  _dataset_exists = function(dataset_id, async) {
    var deferred, exists,
      _this = this;
    if (async == null) {
      async = true;
    ...
    */
    ...
    bamboo.query_info(dataset_id, async).then(function(response) {
      exists = true;
  ...
  Dataset = (function() {
    ...
    Dataset.prototype.load_from_url = function(url, sync_cb) {
      ...
      bamboo.create_dataset(this.url, async).then(function(response) {
    ...
    Dataset.prototype.join = function(right, on_column, sync_cb) {
      var async, for_what,
        _this = this;
      if (sync_cb == null) {
        sync_cb = false;
      ...
      */
      ...
      async = !!sync_cb;
      for_what = "joined datasets " + this.id + " and " + right;
      if (this.joins == null) {
        this.joins = [];
      }
      bamboo.join(this.id, right, on_column, async).then(function(response) {
        _this.joins.push(response.id);
        _this.on_success(for_what, response, sync_cb);
      }, function(e) {
        _this.on_failure(for_what, e, sync_cb);
      });
    ...
    Dataset.prototype.merge = function(datasets, sync_cb) {
      var async, for_what,
        _this = this;
      if (sync_cb == null) {
        sync_cb = false;
      ...
      */
      ...
      bamboo.merge(datasets, async).then(function(response) {
        _this.on_success(for_what, response, sync_cb);
      }, function(e) {
        _this.on_failure(for_what, e, sync_cb);
      });
    ...
    Dataset.prototype.update = function(rows, sync_cb) {
      ...
      */
      ...
      async = !!sync_cb;
      for_what = "updating dataset " + this.id;
      bamboo.update(this.id, rows, async).then(function(response) {
        _this.on_success(for_what, response, sync_cb);
      }, function(e) {
        _this.on_failure(for_what, e, sync_cb);
      });
    ...
    Dataset.prototype["delete"] = function(sync_cb) {
      var async, deleted, for_what,
        _this = this;
      if (sync_cb == null) {
        sync_cb = false;
      }
      async = !!sync_cb;
      for_what = "deleting dataset " + this.id;
      deleted = false;
      bamboo.delete_dataset(this.id, async).then(function(response) {
        deleted = true;
        _this.on_success(for_what, response, sync_cb);
      }, function(e) {
        _this.on_failure(for_what, e, sync_cb);
