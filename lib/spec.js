...
(function() {
  ...
  describe("updates, join, merge", function() {
    ...
    it("can update data in an dataset", function() {
      ...
      return this.dataset.update([update_data]);
    });
    it("can merge a few datasets together", function() {
      var dataset_id, datasets;
      this.dataset_merge = new bamboo.Dataset({
        url: file_url("jan_15", "merge"),
        autoload: true
      });
      datasets = [this.dataset, this.dataset_merge];
      dataset_id = this.dataset.id;
      this.dataset.merge(datasets, function(merged) {
        return expect(merged.id).not.toEqual(dataset_id);
      });
      return this.dataset_merge["delete"]();
    });
    it("can join two datasets on a certain column", function() {
      runs(function() {
        return this.dataset_join = new bamboo.Dataset({
      ...
      waits(2000);
      return runs(function() {
        this.dataset.join(this.dataset, this.dataset_join, "name");
